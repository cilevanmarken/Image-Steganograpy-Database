...
def verify_and_initialize():
    global path, secret_message, destination_path, compression, encoding, embedding, secret_key, path_token, secret_path, image_name
    quality = 75
    ...
    if args.encode:
        ...
        dest = destination_path + image_name + ".png"
        print(current_time, f"Image {image_name} from {path} saved in {dest} with {secret_path}")
...
def encode():
    global secret_image, path_token, image_name
    if compression == "y":
        if encoding == "s":
    ...
    try:
        if compression == "y":
            ...
            imageio.imwrite(str(destination_path) + image_name + ".png", secret_image.astype(numpy.uint8))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--secret_path',
                        help="Destination file for secret image")
    parser.add_argument('--image_name',
                        help="Destination name for secret image")
